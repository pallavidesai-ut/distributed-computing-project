...
def build_report(rows: list[dict[str, object]], output_dir: Path, experiment_config: dict[str, object]) -> None:
    ...
    lines = [
        ...
        "| --- | --- | --- | --- |",
        "| VV | Exact per-object vector over client/session actors | Full ancestry precision with the simplest semantics | Metadata grows with the number of distinct writers touching an object |",
        ...
        f"- Client/session actor pool: {experiment_config['client_count']}",
        ...
        "",
        "Each run mixes background read-then-write traffic with explicit hot-key contention bursts and later merge writes. Exact VV uses client/session actors, vnode-VV uses production-style replica actors, and DVV/lease-DVV use replica dots. This yields both a fairness track and a realism track in one experiment matrix.",
        "",
        "## Aggregated Findings",
        "",
    ]
    ...
    lines.extend(
        [
            "## Interpretation",
            "",
            "- `vv` is the exact fairness baseline. It shows the metadata cost of preserving full object ancestry with client/session actors.",
            ...
            "- `dvv` should match `vv` on correctness while staying much closer to the metadata footprint of replica-bounded tracking.",
